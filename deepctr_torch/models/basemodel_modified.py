...
class BaseModel(nn.Module):
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            if do_validation:
                ...
                for name, result in eval_batch_variance.items():
                    epoch_logs["val_batch_"+name] = result
            # verbose   
            if verbose > 0:
                ...
                eval_str = "{0}s - loss: {1: .4f} - batch_variance: {2: .4f}".format(
                    epoch_time, epoch_logs["loss"],epoc_logs["batch_variance"])
                ...
                if do_validation:
                    for name in self.metrics:
                        eval_str += " - " + "val_" + name + \
                                    ...
                                    " - " + "val_batch_" + name + \
                                    ": {0: .4f}".format(epoch_logs["val_batch_" + name])
                print(eval_str)
    ...
    def evaluate(self, x, y, batch_size=256):
        ...
        eval_loss_vector = self.loss_func(pred_ans, y.squeeze(), reduction='none')

        eval_batch_variance = torch.std(eval_loss_vector)
        eval_result = {}
        for name, metric_fun in self.metrics.items():
