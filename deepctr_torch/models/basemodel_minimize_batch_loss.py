...
#                         import pdb
#                         pdb.set_trace()
                        loss_vector = loss_func(y_pred, y.squeeze(), reduction='none')
                        loss = torch.sum(loss_vector)
                        # reduction = 'none' ==> return a vector
                        reg_loss = self.get_regularization_loss()
                        batch_variance_loss = torch.std(loss_vector)
                        total_loss = loss + reg_loss + self.aux_loss + lamda*batch_variance_loss
                    
                        
                        loss_epoch += loss.item()
                        total_loss_epoch += total_loss.item()
                        # mark
                        total_batch_variance += batch_variance_loss.item()
                        total_loss.backward()
                        optim.step()
            ...
            epoch_logs["batch_variance"] = total_batch_variance / sample_num
            for name, result in train_result.items():
                epoch_logs[name] = np.sum(result) / steps_per_epoch

            if do_validation:
                eval_result,eval_batch_variance = self.evaluate(val_x, val_y, batch_size)
                for name, result in eval_result.items():
                    epoch_logs["val_" + name] = result
                for name, result in eval_batch_variance.items():
                    epoch_logs["val_batch_loss"] = result
            # verbose   
            if verbose > 0:
                epoch_time = int(time.time() - start_time)
                print('Epoch {0}/{1}'.format(epoch + 1, epochs))

                eval_str = "{0}s - loss: {1: .4f} - batch_variance: {2: .4f}".format(
                    epoch_time, epoch_logs["loss"],epoch_logs["batch_variance"])
                ...
                if do_validation:
                    for name in self.metrics:
                        eval_str += " - " + "val_" + name + \
                                    ...
                                    " - " + "val_batch_loss"  + \
                                    ": {0: .4f}".format(epoch_logs["val_batch_loss"].item())
                print(eval_str)
    ...
    def evaluate(self, x, y, batch_size=256):
        ...
        pred_ans = torch.from_numpy(pred_ans.astype('float'))
        y = torch.from_numpy(y.astype('float'))
#         import pdb
#         pdb.set_trace()
        loss_func = self.loss_func
        eval_loss_vector = loss_func(pred_ans.squeeze(), y.squeeze(), reduction='none')

        batch_variance =torch.std(eval_loss_vector)
        eval_result = {}
        eval_batch_variance = {}
        for name, metric_fun in self.metrics.items():
            eval_result[name] = metric_fun(y, pred_ans)
            eval_batch_variance[name] = batch_variance
        return eval_result,eval_batch_variance
