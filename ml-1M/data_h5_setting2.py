...
sys.path.insert(0, os.path.realpath('../examples'))
...
le.fit(users['zip'].to_numpy())
zip_transformed = le.transform(users['zip'].to_numpy())
users['zip'] = zip_transformed.tolist()
...
user_num = ratings.value_counts('user_id')
vec_list = []
for e in movies['title'].to_numpy():
    doc = nlp(e)
    vec = list(doc.vector)
    vec_list.append(vec)
movies['title_vec'] = vec_list

genres_list = []
for e in movies['genres'].to_numpy():
    genres_list.extend(e.split('|'))
leg = preprocessing.LabelEncoder()
leg.fit(genres_list)
genres_transform_list = []
genres_len = len(np.unique(leg.transform(genres_list)))
for e in movies['genres'].to_numpy():
    tmp = np.zeros(genres_len)
    tmp[leg.transform(e.split('|'))] = 1
    genres_transform_list.append(tmp.astype('int32').tolist())
movies['genres_vec'] = genres_transform_list
...
data = data[['user_id','movie_id', 'rating', 'timestamp', 'gender','age','occupation','zip', 'title','genres', 'title_vec', 'genres_vec']] # select the features we need

################################## remove the invalid zero ###########################################
valid_list = []
...
for i in tqdm(range(len(data))):
    if uid[i] != len(valid_list):  #new user
...
data = data[['user_id','movie_id', 'rating', 'gender','age','occupation','zip', 'title_vec','genres_vec']] # select the features we need
# 取前 1W
#data = data[:10000]
num_count = data['user_id'].value_counts()  
...
for i in tqdm(range(1,user_num+1)):
    ...
    for index in range(1,hist_len):  #遍历该user的每一条记录,从第1条开始(0,1,2,3...)
        ...
        import pdb
        oua = one_user[index][3:]
        
        # attr_tmp = [np.array(oua[:4]), oua[4], np.pad(oua[-1], (0,10-len(oua[-1])), constant_values=0)]
        pdb.set_trace()
        attr_tmp = [np.array(oua[:4]), oua[4], np.array(oua[5])]
        attr_tmp = np.concatenate(attr_tmp)
        attrs.append(attr_tmp.astype('float32'))
        import pdb
        if index == 1:
...
attrs_ndf = f1.create_dataset("attrs", data=attrs)


from tensorflow.python.keras.preprocessing.sequence import pad_sequences
hist_mid_np = pad_sequences(hist_mid,maxlen = C.max_pad_length,padding = 'pre')
